...
export default function AllProducts() {
  ...
  // Generate deterministic views/sold to look highly realistic and match screenshots
  const getMockViews = (id: string) => {
    const code = id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    return (code % 45) + 2; // Returns 2 - 46 views
  };

  const getMockSold = (id: string) => {
    const code = id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
    return (code % 28) + 1; // Returns 1 - 29 sold
  };
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {products.map((prod) => {
                  ...
                  return (
                    <div
                      key={prod._id}
                      className="flex flex-col bg-white rounded-3xl border border-slate-200/60 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group relative justify-between min-h-[380px]"
                    >
                      {/* Top Badges & Fav */}
                      ...
                      <div className="p-4 flex-1 flex flex-col justify-between space-y-3 text-left">
                        <div className="space-y-1.5">
                          <span className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">
                            {(prod.brand && typeof prod.brand === 'object') ? prod.brand.name : (prod.brand || 'Laptop')}
                          </span>
                          ...
                          <p className="text-[11px] text-slate-450 line-clamp-2 text-slate-500">
                            {prod.shortDesc || prod.description || 'Sản phẩm cao cấp chất lượng vượt trội.'}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-3 text-[11px] text-slate-500 flex-wrap py-1.5 border-t border-slate-100">
                          <div className="flex items-center gap-0.5">
                            ...
                            <span className="font-bold text-slate-700">{prod.ratingsAverage || 5.0}</span>
                            <span className="text-slate-400">({prod.ratingsQuantity || 0})</span>
                          </div>
                          <span className="text-slate-300">|</span>
                          <div className="flex items-center gap-1">
                            <Eye className="w-3.5 h-3.5 text-slate-400" />
                            <span>{getMockViews(prod._id)}</span>
                          </div>
                          <span className="text-slate-300">|</span>
                          <div className="flex items-center gap-1">
                            <ShoppingBag className="w-3.5 h-3.5 text-slate-400" />
                            <span>Đã bán {getMockSold(prod._id)}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-end justify-between pt-1 relative pr-12">
                          ...
                          <button
                            ...
                            className={`absolute bottom-0 right-0 w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-300 cursor-pointer ${
                              isOutOfStock 
              ...
              <div className="flex flex-col gap-4">
                {products.map((prod) => {
                  ...
                  return (
                    <div
                      key={prod._id}
                      className="bg-white rounded-3xl border border-slate-200/60 overflow-hidden hover:shadow-lg transition-all duration-300 group flex flex-col md:flex-row relative p-4 gap-6 items-center md:items-start"
                    >
                      {/* Left: Fav and Image */}
                      ...
                      <div className="flex-grow flex flex-col justify-between space-y-3 text-left w-full">
                        <div className="space-y-1">
                          ...
                          <p className="text-xs text-slate-550 line-clamp-2 text-slate-500">
                            {prod.shortDesc || prod.description || 'Sản phẩm cao cấp chất lượng vượt trội.'}
                          </p>
                        </div>
                  ...
                  {brands.map((b) => {
                    ...
                    return (
                      <button
                        key={b._id}
                        onClick={() => {
                          setSelectedBrand(isSelected ? 'all' : b.slug);
                          setIsMobileFilterOpen(false);
                        }}
